...
표 2의 통제변수는 BaTIS의 추정 모형과 같은 셋이다. Li et al. (2025)는 여기에서 식민 관계를 빼고 법체계의 공통 기원, 무역협정, EU 동반 가입을 더한 조합을 썼다. 같은 조합으로 식 (1)을 다시 추정하면 거리 탄력성의 절댓값은 여덟 범주 모두에서 작아진다. 가장 큰 몫은 EU 동반 가입이 설명하는데, 이 변수 하나만 더해도 절댓값이 범주에 따라 0.04\~0.07 줄어든다. EU 역내 교역은 거리가 짧고 교역이 많으므로 그 효과가 거리 계수에 섞여 있었던 것이다. 이 조합에서 운송(-0.533), 여행(-0.791), 금융(-0.560)은 Li et al. (2025)가 보고한 범주 평균(-0.54, -0.73, -0.50)과 가깝다. 반면 통신·컴퓨터·정보(-0.587)와 기타 사업(-0.475)은 그들의 -0.37과 -0.35보다 여전히 크고, 값을 균형치 대신 수출국 보고를 우선한 보고치로 바꾸어도 차이는 크게 줄지 않는다. 남은 차이는 그들이 UNCTAD-WTO와 UN Comtrade 자료를 더해 보고국과 기간을 넓힌 데서 올 수 있으나, 본 연구는 이를 확인하지 않았다.
