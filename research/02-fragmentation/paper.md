...
앞 절에서 본 하락은 서비스를 하나로 묶은 값이라 어느 범주에서 나왔는지는 드러나지 않는다. 범주로 나누어 보는 것은 서론에서 언급한 두 번째 가설을 검증하기 위해서다. 그 가설의 근거는 디지털 서비스를 겨냥한 규제가 2018년 이후 늘었다는 데 있으므로, 가설이 맞다면 서비스 전체의 하락은 디지털로 전달되는 범주에서 나와야 하고 그 범주의 블록 간 비중이 먼저 내려가 있어야 한다. 서비스를 다섯 범주로 나누고 전체 변화를 범주별 기여로 분해했다.
