...
표에서 보듯이 서비스 전체의 하락은 여행 한 범주에서 나온다. 여행의 기여는 -1.99%p로 서비스 전체의 하락 0.84%p보다 크고, 나머지 네 범주의 기여를 합하면 양수다. 디지털로 전달되는 두 범주가 특히 그렇다. ICT가 +0.64%p, 기타 디지털이 +0.29%p로 둘이 합쳐 서비스 전체의 블록 간 비중을 0.93%p 끌어올렸다. 두 범주의 상승은 교역 상대가 아니라 규모에서 온다. ICT의 범주 내 블록 간 비중은 21.56%에서 21.74%로 사실상 그대로인데 교역 비중이 8.86%에서 11.75%로 커졌고, 두 범주의 범주 내 효과는 +0.02%p와 -0.06%p로 0 부근이다. 여행은 그 반대로 블록 간 비중이 27.83%로 높은 범주인데 교역 비중이 21.73%에서 16.36%로 줄었고 범주 안의 비중도 24.79%로 내려갔다. 서비스 전체의 하락 0.84%p는 이렇게 방향이 반대인 두 움직임이 상쇄되고 남은 값이어서, 그 한 수치만 보면 디지털 범주가 오히려 올랐다는 사실이 가려진다.
...
여행의 하락에는 지정학적 분절과 구분되지 않는 요인이 섞여 있다. 중국은 2023년 1월까지 방역을 이유로 출입국을 통제했으므로 2022년의 급락에는 코로나 정책이 섞여 있다. 그래서 사후 기간을 중국의 국경 재개 이후인 2023\~2024년으로만 잡아 다시 분해했다. 여행이 끌어내리는 폭은 1.57%p(구성 -1.13, 범주 내 -0.44)로 줄어들었다. 그러나 이 폭은 그 기간 서비스 전체의 하락 0.82%p보다 여전히 크다. 여행 교역의 위축이 방역 통제 해제 뒤에도 남아 있다는 뜻이지만, 그것이 중국인 해외여행의 더딘 회복과 정치적 요인 가운데 무엇에서 오는지는 이 자료로 구분되지 않는다.
...
$S$의 하락이 교역 상대의 재편인지, 한 블록의 몫이 줄어서 생긴 것인지는 집약도 $I$로 구분할 수 있다. 표 3은 II.3절에서 정의한 $I = S/E$를 사전 기간과 사후 기간에 대해 구한 것이다. $S$ 열은 식 (1)의 블록 간 교역 비중으로 사전 값은 표 1의 사전 수준과 같고, $E$ 열은 수출 쪽 몫과 수입 쪽 몫을 그대로 둔 채 수출국과 수입국을 무작위로 짝지었을 때 기대되는 블록 간 비중이며, $I$ 열은 앞의 둘을 나눈 값으로, 1이면 관측된 블록 간 교역이 그 기댓값과 같고 1을 밑돌면 기댓값보다 적다. 마지막 열은 $I$의 사후 값이 사전 값에서 몇 퍼센트 달라졌는지로, 값이 음수이면 두 블록의 몫이 달라진 것만으로 기대되는 것보다 블록 간 교역이 더 줄었다는 뜻이고 양수이면 그 반대다.
...
첫 번째 가설, 관세와 원산지 규정을 받지 않는 서비스에서는 분절화가 약하다는 가설에 대한 답은 측정 방식에 따라 달라진다. 블록 간 비중으로 측정하면 이 가설은 지지되지 않는다. 서비스의 비중 하락은 폭으로는 상품보다 조금 작지만 상대적 크기로는 같다. 다만 III.2절에서 적었듯이 두 하락은 모두 0과 구별되지 않으므로, 이 대목이 뒷받침하는 것은 서비스의 하락이 상품보다 작다는 증거가 없다는 데까지다. 블록별 몫의 변화를 통제한 집약도로 측정하면 이 가설과 맞는 결과가 나온다. 서비스에서는 몫의 변화를 넘는 재편이 보이지 않는다. 상품에서는 약하게 보이지만 그것도 재수출 조정 균형치에서만 나타나고 다른 두 상품 계열에서는 나타나지 않으며, 계열별 비교는 이 장의 2절에 있다. 두 측정이 다른 답을 내는 것은 서비스의 하락이 교역 상대의 재편이 아니라 중국 측 블록의 서비스 교역 위축에서, 구체적으로는 여행의 위축에서 나왔기 때문이다.
...
폭이 비슷하다고 해서 두 교역에서 같은 일이 벌어진 것은 아니다. 서비스의 하락은 여행 한 범주에서 나왔고, 블록별 몫의 변화를 통제한 집약도로 보면 그 하락이 남지 않는다. 교역 상대가 블록을 따라 재편되어서가 아니라, 블록 간 비중이 높은 여행 교역이 위축되면서 블록을 넘는 교역도 함께 줄어든 것이다. 디지털 범주는 반대로 움직였다. ICT의 블록 간 비중은 데이터 규제가 늘어난 시기에도 그대로였고, ICT와 기타 디지털 서비스의 성장은 서비스 전체의 블록 간 비중을 0.93%p 끌어올렸다. 서비스 전체의 비중 하나만 보면 이 상승이 여행의 위축에 가려진다.
...
본 연구가 내놓은 것은 인과관계가 아니라 자료에서 관찰한 사실이며 세 가지 단서를 갖는다. 첫째, 수출국을 복원추출한 95% 구간은 서비스의 하락과 상품의 하락, 그리고 두 하락의 차이를 모두 0과 구별하지 못한다. 본 연구가 주장하는 것은 하락의 크기가 아니라 블록 정의와 교역 계열을 바꾸어도 방향이 같다는 점이다. 둘째, 블록을 사전 기간의 이념점수로 정했으므로 러시아가 두 블록 어느 쪽에도 들어 있지 않다. 대체안 세 가지에서는 하락이 더 커지므로 기준안은 분절화를 작게 잡는 선택이다. 셋째, 사후 관측이 세 해뿐이다. 남은 과제는 둘이다. 비동맹 경제가 두 블록 양쪽과 하는 교역의 점유율을 따로 측정해 연결국의 역할을 보는 일, 그리고 여행의 위축에서 방역 정책의 잔여 효과와 정치적 요인을 구분하는 일이다.
