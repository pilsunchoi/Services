...
변화로 보면 서비스에서는 집약도의 하락이 없다. $S$는 사전 대비 4.0% 낮아졌지만 $E$가 5.6% 낮아져, 기대되는 비중이 관측된 비중보다 더 크게 줄었기 때문이다. $E$가 내려갔다는 것은 두 블록의 몫이 미국 측으로 더 기울어 상대를 무작위로 짝지어도 블록을 넘는 짝이 전보다 덜 나오게 되었다는 뜻이다. 실제로 두 블록에 속한 경제끼리의 서비스 교역 가운데 중국 측이 수입한 몫은 15.98%에서 14.38%로 줄었고, 수출한 몫은 12.06%에서 11.85%로 거의 그대로다. 서비스의 블록 간 비중 하락은 교역 상대가 재편되어서가 아니라 중국 측 블록이 사들이는 서비스가 줄어든 데서 나왔다. 앞 절에서 본 여행 교역의 위축이 그 경로이며, 그 위축이 중국 측의 수입에서 나타난다는 점도 여기에 들어맞는다. 상품에서는 반대로 $S$가 3.2% 낮아지는 동안 $E$가 1.9% 낮아지는 데 그쳐 몫을 통제한 뒤에도 하락이 남지만, 그 크기는 1.39%다. 이 하락은 상품 계열을 재배분 이전의 총 균형치나 BACI로 바꾸면 사라지며, 세 계열의 비교는 IV.3절에 있다.

다만 집약도가 무엇을 통제하는지를 함께 보아야 한다. 제재나 그 밖의 분절화 정책이 한 블록의 교역 규모 자체를 줄였다면 그 감소도 분절화의 결과인데, 집약도는 그 부분을 기댓값 쪽에 넣어 지운다. 그러므로 이 수치는 서비스에 분절화가 없다는 증거가 아니라, 서비스에서 관측된 비중 하락이 두 블록의 몫 변화로 모두 설명된다는 증거다.
