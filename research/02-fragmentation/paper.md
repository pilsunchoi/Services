...
가설은 서로 반대 방향으로 두 가지를 세울 수 있다. 첫째, 서비스는 관세와 원산지 규정을 받지 않으므로 교역 상대를 정책으로 구분할 수단이 상품보다 적고, 그만큼 분절화가 약하게 나타날 수 있다. 둘째, 데이터 현지화, 클라우드 규제, 보안 심사처럼 디지털 서비스를 겨냥한 조치는 2018년 이후 늘었으므로, 디지털로 전달되는 서비스가 오히려 상품보다 먼저 분절될 수 있다. 다만 두 가설이 겨냥하는 층위는 다르다. 첫째는 서비스 전체가 상품과 비교해 어떻게 움직이는가를 말하고, 둘째는 서비스 안에서 어느 범주가 먼저 움직이는가를 말한다. 서비스 전체의 블록 간 비중은 범주의 구성에도 반응하므로 한 범주의 부침이 다른 범주의 움직임을 덮을 수 있고, 그래서 총량 하나로는 두 가설을 함께 가릴 수 없다. 본 연구는 서비스 전체를 상품과 같은 조건에서 비교한 뒤 서비스를 범주로 나누어 두 가설에 차례로 답하며, 어느 쪽이 자료와 맞는지를 기술적 사실의 수준에서 확인한다.
