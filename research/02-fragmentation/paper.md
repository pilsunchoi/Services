...
여기에서 $\bar{w}_k$와 $\bar{s}_k$는 각각 $w_k$와 $s_k$를 사전과 사후 두 기간에 대해 평균한 값이다. 첫 항은 구성 효과로 블록 간 비중이 높은 범주의 교역이 커지거나 작아져서 생기는 변화이고, 둘째 항은 범주 내 효과로 같은 범주 안에서 교역 상대가 재편되어 생기는 변화다. 표 2가 그 분해다. 앞의 네 열은 각 범주의 교역 비중 $w_k$와 범주 내 블록 간 비중 $s_k$를 사전과 사후로 보이고, 변화 열은 $s_k$의 변화이며, 마지막 두 열이 식 (2)의 첫째 항과 둘째 항이다.
