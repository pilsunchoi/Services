...
지경학적 분절화(geoeconomic fragmentation)는 시장 조건이 아니라 지경학적 요인, 곧 안보와 동맹 관계를 고려한 정책으로 세계 경제의 통합이 후퇴하는 현상을 가리킨다. 관세와 수출 통제, 투자 심사, 제재가 그런 정책이다. 세계 교역의 규모 자체는 줄지 않았으므로 분절화를 다루는 국제경제학 문헌이 주목하는 것은 총량이 아니라 교역 상대의 재편이다. Gopinath et al. (2025)는 러시아의 우크라이나 침공 이후 지정학적으로 먼 블록 사이의 상품 교역이 같은 블록 안의 교역보다 약 12% 더 줄었다고 추정했고, WTO (2023)는 UN 총회 표결로 두 블록을 가정했을 때 침공 이후 블록 사이의 상품 교역이 블록 안의 교역보다 4~6% 느리게 늘었다고 보고했다. 두 연구가 다룬 것은 모두 상품 교역이다. 서비스 교역에서 같은 재편이 나타나는지는 자료의 제약 때문에 거의 검증되지 않았다.
...
블록은 네 가지로 정의한다. 이념점수 기반 분절화 측정이 추정 기간과 표결 선택에 민감하다는 비판(Airaudo et al., 2025)에 대응하려면 단일 정의에 기대지 않아야 하기 때문이다. 이념점수는 Bailey et al. (2017)의 UN 총회 최종 통과 표결 기반 추정치(1946~2025)를 쓴다. 이 값은 1차원이며 높을수록 미국과 서방 민주국가에 가깝다. 표 1은 네 가지 정의를 함께 제시한 것이다. 규칙 열은 두 블록을 구분하는 기준이고, 오른쪽 세 열은 그 기준으로 각 블록에 배정된 경제의 수다.
...
자료: Bailey et al. (2017)의 이념점수 2026년판, 러시아 정부 지령 430-r(2022-03-05), UN 총회 결의 ES-11/1 표결 기록에서 작성.
