...
본 연구는 사전 기간의 이념점수로 정한 기준안 하나만 썼다. 정치적 위치는 교역 관계의 영향을 받으므로, 사후 정보를 쓰는 정의에서는 분류와 결과가 함께 움직인다. 사전 기간으로 고정한 것은 그 경로를 막기 위해서이지만, 그 대신 뒤늦게 어느 쪽에 가까운지가 뚜렷해진 나라가 비동맹으로 남는다. 러시아가 그 경우다. 이념점수로 블록을 구분하는 방식 자체가 추정 기간과 표결 선택에 민감하다는 비판도 있다(Airaudo et al., 2025).
...
본 연구는 상품 교역에서 확인된 지경학적 분절화가 서비스 교역에서도 나타나는지를 물었다. 같은 국가쌍에 고정한 9,192개 수출 흐름에서 두 교역을 같은 조건에서 측정한 결과, 블록 간 교역 비중은 사후 기간에 서비스에서 0.84%p, 상품에서 1.02%p 낮아졌다. 하락의 폭은 서비스 쪽이 작지만 서비스의 비중 수준이 낮은 데서 출발하므로 상대적 크기로는 둘이 같다. 미중 관세 분쟁기에는 두 교역 모두 변화가 없었다. 하락이 나타난 시점은 달라서 상품은 2022년 이후에 내려갔고 서비스는 코로나 충격기인 2020년에 이미 한 단계 낮아졌다.

폭이 비슷하다고 해서 두 교역에서 같은 일이 벌어진 것은 아니다. 관세와 원산지 규정을 받지 않는 서비스에서 분절화가 약하다는 첫 번째 가설에 대한 답은 측정 방식에 따라 달라진다. 블록 간 비중으로 측정하면 지지되지 않지만, 블록별 몫의 변화를 통제한 집약도로 측정하면 이 가설과 맞는 결과가 나온다. 두 측정이 다른 답을 내는 것은 서비스의 하락이 교역 상대의 재편이 아니라 중국 측 블록의 서비스 교역 위축에서, 구체적으로는 여행의 위축에서 나왔기 때문이다. 블록 간 비중이 높은 여행 교역이 줄면서 블록을 넘는 교역도 함께 줄어든 것이다. 디지털 서비스가 상품보다 먼저 분절된다는 두 번째 가설은 지지되지 않는다. ICT의 블록 간 비중은 데이터 규제가 늘어난 시기에도 그대로였고, ICT와 기타 디지털 서비스의 성장은 서비스 전체의 블록 간 비중을 0.93%p 끌어올렸다. 서비스 전체의 비중 하나만 보면 이 상승이 여행의 위축에 가려진다.

이 결과는 분절화를 블록 간 교역 비중 하나로 측정하는 관행에 유보를 남긴다. 비중은 교역 상대의 선택만이 아니라 각 블록이 교역에서 차지하는 몫에도 반응하므로, 한쪽 블록의 교역이 위축된 기간에는 두 가지가 섞인다. 서비스와 상품의 비중 하락은 비슷하지만, 몫의 변화를 통제한 집약도는 서비스에서 1.71% 오르고 상품에서 1.39% 내려 부호가 반대다. 여행처럼 규모가 크게 오르내리는 범주를 포함한 교역에서는 두 지표가 서로 다른 결론을 낸다. 상품 교역에서 세워진 측정 관행을 서비스로 옮길 때 몫의 변화를 통제한 지표를 함께 보아야 하는 이유다.
