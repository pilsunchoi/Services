...
본 연구는 블록을 사전 기간의 이념점수로 정한 기준안 한 가지만 썼다. 사후 기간의 점수를 쓰면 블록 분류가 측정하려는 교역 변화와 함께 움직여 둘을 구분할 수 없기 때문이다. 그 대신 러시아처럼 나중에야 한쪽으로 기운 것이 뚜렷해진 나라가 비동맹에 남는다. 이념점수로 블록을 나누는 방식 자체가 추정 기간과 표결 선택에 민감하다는 비판도 있다(Airaudo et al., 2025). 그래서 다른 정의로도 같은 값을 구해 결과가 정의에 달려 있는지 확인한다.
