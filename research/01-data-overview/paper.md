...
OECD와 WTO가 함께 만드는 BaTIS는 이 빈칸을 채워 완전한 행렬을 만드는 것을 목표로 한다. 각국이 보고한 값을 모으고, 보고되지 않은 칸은 중력모형(gravity model)으로 추정하며, 수출국 보고와 수입국 보고가 어긋나는 곳은 양쪽을 화해시켜 하나의 값으로 만든다. 이 작업의 결과가 205개 경제와 그 상대국을 EBOPS 2010 기준 27개 항목으로 덮는 2005\~2024년 행렬이다. 같은 기관이 같은 철학으로 만든 상품 교역판이 BIMTS이고, 이쪽은 1995\~2024년을 HS2017 기준으로 덮는다.
...
용어는 원자료의 구분을 그대로 쓴다. 보고치는 통계당국이 실제로 보고한 값, 조정치는 거기에 추정과 조정을 더해 내부 정합을 맞춘 값, 균형치는 수출국 보고와 수입국 보고를 화해시킨 값이다. 영문 문헌의 reported, final, balanced에 대응하며, 같은 거래를 상대국이 적은 수입국 보고는 문헌에서 거울상 통계(mirror statistics)라 부른다.
...
| `balanced_value` | 수출국 보고와 수입국 보고를 화해시킨 값 | 98.9% | 나라쌍 행렬이 닫혀야 하는 분석 |
...
균형화는 같은 거래에 대한 두 나라의 보고가 어긋날 때 하나의 값으로 만드는 절차다. A국이 B국에 판 서비스 수출과 B국이 A국에서 산 서비스 수입은 원리상 같은 거래이지만 실제 보고는 자주 다르다. BaTIS는 두 값의 대칭 정도를 가중치로 삼아 가중평균을 구하고 그 값을 양쪽에 같이 써 넣는다. 결과적으로 균형치에서는 수출국 보고와 수입국 보고가 정확히 같아진다.
...
**그림 1. 세계 수출에서 상품과 서비스의 규모, 서비스 비중 (균형치, 개별 경제쌍 합계)**
...
**그림 2. 세계 서비스 수출의 항목별 비중 (균형치, 비중 상위 6개 항목)**
...
**표 7. 주요 보고국의 상대별 보고율 (2019년, 총서비스 수출, 200개 상대국 기준)**
...
균형화가 왜 필요한지는 양쪽이 모두 보고한 흐름을 보면 알 수 있다. 2019년 총서비스에서 수출국의 수출 보고와 상대국의 수입 보고가 둘 다 존재하고 모두 양수인 흐름은 1,719개다. 같은 해 개별 경제 사이의 방향별 칸이 4만 개이므로 양쪽 보고가 모두 있는 경우는 4.3%에 그친다. 그림 4는 이 1,719개 흐름에서 두 보고치의 차이를 두 값의 평균 대비 백분율로 나타낸 분포이며, 세로 파선이 중위값이다.

**그림 4. 양쪽이 모두 보고한 흐름의 비대칭 분포 (2019년, 총서비스, 1,719개)**

![양쪽이 모두 보고한 흐름의 비대칭](img/fig4_asymmetry.png)

중위값이 40.4%이고 사분위 범위가 18.3\~76.8%다. 차이가 25%를 넘는 흐름이 67.3%, 50%를 넘는 흐름이 42.5%다. 같은 거래를 두 나라가 보고했는데 그 값이 절반 가까이 차이 나는 경우가 흔하다는 뜻이다. 상품 교역에서도 두 보고의 비대칭은 알려진 문제이지만 서비스에서는 그 크기가 훨씬 크다. 서비스 거래는 통관 기록이 없어 조사와 추정에 의존하고, 중개 거래와 거주자 판정의 차이가 양국 통계에 다르게 반영되기 때문으로 보인다.
...
**그림 5. 한국의 서비스 수출입과 상품 수출 (균형치, 상품은 재수출 조정)**
...
두 자료를 함께 담은 이점은 같은 나라쌍에서 서비스와 상품을 비교할 수 있다는 것이다. 2024년에 서비스와 상품이 모두 양수인 흐름은 2만 8,765개이고, 두 금액에 로그를 취한 상관계수는 0.793이다. 상품 교역이 많은 흐름에서 서비스 교역도 많다는 관계가 뚜렷하지만 완전하지는 않다.

이 관계를 해석할 때 두 가지를 주의해야 한다. 첫째, 두 자료의 결측 규약이 다르다. BaTIS는 모든 칸을 채우고 BIMTS는 0인 흐름을 행에서 제외하므로, 둘을 결합하면 상품 교역이 관측된 흐름만 남는 선택이 일어난다. 둘째, 서비스 금액의 대부분이 추정이므로 이 상관의 일부는 중력모형이 상품 교역과 같은 설명변수(거리, 규모, 인접성)를 쓴 결과일 수 있다. 관측된 두 계열의 관계가 아니라 추정 절차가 만들어 낸 관계일 가능성을 배제하려면 보고치만으로 같은 상관을 다시 측정해야 한다. 이 문서는 그 작업을 수행하지 않았다.
